...
//----------------------------------------------------------------------//
    ...
    for (int i = 0; i < n; ++i) {
        ...
        if (i < n - 1) {
            Matrix diag_L = std::get<0>(extractLU(diag));
            Matrix diag_U = std::get<1>(extractLU(diag));
    ...
    if (id == aux_id) {
        std::cout << "Time spent: " << time(start, end) << " ms" << std::endl;
        std::cout << "Precision: " << precision(old_full_mtx, full_mtx) << std::endl;
    }
